...
public final class FieldInspections extends Suite {
    ...
    public Predicate<Reference> nonFinal() {
        return and(field(isNotStatic(),
                         isNotAnnotatedWith(
                                 "org.springframework.beans.factory.annotation.Value"),
                         isNotFinal(),
                         isNotTransient(),
                         declaringClass(isNotSyntheticClass()),
                         modifiedByNonConstructor().or(isUpdatedInOtherClasses())
                   ),
     ...
     */
    @ProbableError(
            "Static non-final instance field can be modified: stateful shared object")
    @Scopes({"static", "singleton", "session"})
    public Predicate<Reference> nonFinalStatic() {
        return and(field(isStatic(),
                         ...
                         isNotFinal(),
                         declaringClass(isNotSyntheticClass()),
                         modifiedByNonClassInit().or(isUpdatedInOtherClasses())
        ));
    }
    ...
    public Predicate<Reference> nonSerializableField() {
        return and(field(isNotStatic(),
                   ...
                   targetType(isNotPrimitive(), isNot(Serializable.class)),
                   target(Objects::nonNull),
                   referenceTypeIs(ReferenceType.ACTUAL_VALUE),
                   ownerType(hasMethod("writeObject",
    ...
    public Predicate<Reference> nonThreadSafe() {
        return and(referenceTypeIsNot(ReferenceType.THREAD_LOCAL,
                                      ReferenceType.TERMINATED_THREAD_LOCAL,
                                      ReferenceType.WAITING_THREAD_LOCAL
        ), targetType(is(Format.class,
                         Calendar.class,
                         StringBuilder.class,
                         ThreadGroup.class
        )));
    ...
    public Predicate<Reference> transientFieldNotInitialized() {
        return and(field(isNotStatic(),
                   ...
                   ownerType(hasMethod("readObject",
