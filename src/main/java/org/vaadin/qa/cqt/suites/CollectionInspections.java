...
public class CollectionInspections extends Suite {
    ...
    public Predicate<Reference> chmFieldType() {
        return and(targetType(is(ConcurrentHashMap.class)),
    ...
    public Predicate<Reference> chmForSynchronizedHashMap() {
        return and(targetType(is(SYNCHRONIZED_MAP)),
    ...
    public Predicate<Reference> chmKeySetForSynchronizedHashSet() {
        return and(targetType(is(SYNCHRONIZED_SET)),
    ...
    public Predicate<Reference> cowalForSynchronizedArrayList() {
        return and(targetType(is(SYNCHRONIZED_LIST)),
    ...
    public Predicate<Reference> cowalForVector() {
        return and(targetType(is(Vector.class)),
                   referenceTypeIs(ReferenceType.ACTUAL_VALUE,
                                   ReferenceType.POSSIBLE_VALUE
                   )
        );
    ...
    public Predicate<Reference> cslmFieldType() {
        return and(targetType(is(ConcurrentSkipListMap.class)),
    ...
    public Predicate<Reference> emForMapEnum() {
        return and(field(type(is(Map.class)), genericType(0, isEnum())),
                   targetType(isNot(EnumMap.class),
    ...
    public Predicate<Reference> esForSetEnum() {
        return and(field(type(is(Set.class)), genericType(0, isEnum())),
                   targetType(isNot(EnumSet.class),
                              isNot(THREAD_SAFE_COLLECTIONS),
                              isNot(UNMODIFIABLE_COLLECTIONS)
                   ),
                   referenceTypeIs(ReferenceType.ACTUAL_VALUE,
     ...
     */
    @Warning(
            "Modifiable collection exposed via non-private field or non-private getter")
    @Scopes({"static", "singleton", "session"})
    public Predicate<Reference> exposedModifiableCollection() {
        return and(ownerType(isNotPrivateClass()),
                   targetType(is(Collection.class, Map.class),
                              isNot(UNMODIFIABLE_COLLECTIONS)
                   ),
    ...
    public Predicate<Reference> nonThreadSafeCollection() {
        return and(
                ...
                targetType(is(Map.class, Collection.class),
                           isNot(THREAD_SAFE_COLLECTIONS),
                           isNot(UNMODIFIABLE_COLLECTIONS),
                           isNot(SET_FROM_MAP)
                ),
    ...
    public Predicate<Reference> nonThreadSafeCollectionWasNull() {
        return and(
                ownerType(isNotAnnotatedWith(
                        "org.springframework.boot.context.properties.ConfigurationProperties")),
                field(type(is(Map.class, Collection.class),
                           isNot(THREAD_SAFE_COLLECTIONS),
                           isNot(UNMODIFIABLE_COLLECTIONS),
                           isNot(SET_FROM_MAP)
                )),
    ...
    private Predicate<Reference> exposedOrModified() {
        return or(fieldIsExposedForReading(), field(isStatic(),
                                                    type(is(Map.class)),
                                                    calledByNonClassInit(
                                                            MAP_MUTATION_METHODS)
        ), field(isStatic(),
