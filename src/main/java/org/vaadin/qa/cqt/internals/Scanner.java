...
public class Scanner {
     ...
     *
     * @param filter the class filter. Only classes matching filter will be reported.
     */
    ...
    /**
     * Test if object matches filter specified in {@link Scanner#Scanner(Predicate)}
     *
     * @param object the object
    ...
    public void visitEngine() {
        ...
        EngineInstance.get().addSystemObjects((o, s) -> visitObject(unwrap(o), s));
    }
    ...
    private static boolean shouldIgnore(Class<?> clazz) {
        return clazz.isPrimitive()
                ...
                || (clazz.getPackage() != null && clazz.getPackage().getName().startsWith(CodeQualityTestServer.class.getPackage().getName()))
    ...
    private List<Reference> getAllReferences() {
        ...
        for (Map.Entry<Object, ObjectData> entry : visitedObjects.entrySet()) {
            ...
            for (ObjectValue value : data.getValues()) {
                if (ownerInScope && (value.getField() == null || matchesFilter(value.getField().getDeclaringClass()))) {
    ...
    private void propagateScopes() {
        ...
        visitedObjects.entrySet().stream()
                .filter(entry -> entry.getKey() != null && entry.getValue().getEffectiveScope() != null)
                .forEach(entry -> {
                    scopeStats.computeIfAbsent(entry.getValue().getEffectiveScope(), s -> new AtomicInteger(0)).incrementAndGet();
            ...
            if (!objectToVisit.getClass().getName().contains("$Lambda")) { // Do not propagate through lambdas
                for (ObjectValue objectValue : objectData.getValues()) {
                    if (objectValue != null) { // Don't propagate scope to values
                        ...
                        if (other != null
                                && EngineInstance.get().shouldPropagateScope(other.getEffectiveScope(), objectData.getEffectiveScope())
                                && !other.hasOwnScope()) {
                            scopeStats.computeIfAbsent(other.getEffectiveScope(), s -> new AtomicInteger(0)).decrementAndGet();
                            other.setInheritedScope(objectData.getEffectiveScope());
                            scopeStats.computeIfAbsent(other.getEffectiveScope(), s -> new AtomicInteger(0)).incrementAndGet();
    ...
    private void visitObject(Object objectToVisit, String context) {
        ...
        Optional<String> detectedScope = context != null ? Optional.of(context) : Optional.ofNullable(scope.isEmpty() ? null : scope);
